...
class EasySolu(object):
    def longestPalindrome(self, S):
        def getPalindromicSubstr(left, right):
            length = 0
            if left == right:
                length = 1
        ...
        n = len(S)
        max_len = 0
        res_str = "" if len(S) == 0 else S[0]
        for i in range(n - 1):
            length = getPalindromicSubstr(i, i)
            if max_len < length:
                max_len = length
                res_str = S[i - length // 2: i + length // 2 + 1]
            length = getPalindromicSubstr(i, i + 1)
            if max_len < length:
                max_len = length
                res_str = S[i - length // 2 + 1: i + length // 2 + 1]
        return res_str

class ManacherSolu(object):
    def longestPalindrome(self, S):
